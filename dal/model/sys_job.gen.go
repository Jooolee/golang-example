...
type SysJob struct {
	...
	JobGroup       string    `gorm:"column:job_group;primaryKey;default:DEFAULT;comment:任务组名" json:"job_group"`         // 任务组名
	...
	MisfirePolicy  string    `gorm:"column:misfire_policy;default:1;comment:计划执行策略（1多次执行 2执行一次）" json:"misfire_policy"` // 计划执行策略（1多次执行 2执行一次）
	Concurrent     string    `gorm:"column:concurrent;default:1;comment:是否并发执行（0允许 1禁止）" json:"concurrent"`             // 是否并发执行（0允许 1禁止）
